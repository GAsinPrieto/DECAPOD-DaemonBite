...
uint8_t gpBit[GAMEPAD_COUNT_MAX] = {B00000010, B00000100, B00000010, B00000100};//, B00100000};
...
void loop() {
    ...
    if (SISTEMA == GENESIS_)
    {
      ...
      //4player
      DDRB  &= ~B00000110; // inputs
      PORTB |=  B00000110; // enable internal pull-ups      
      ////////////////////////GAP
    ...
    case NES_:
      while (1)
      {
        ...
        if ((micros() - microsButtons) > BUTTON_READ_DELAY)
        {
          ...
          {
            for (gp = 0; gp < GAMEPAD_COUNT_NES-1; gp++)
              //(PINF & gpBit[gp]) ? buttons[gp] &= ~btnBits[btn] : buttons[gp] |= btnBits[btn];
              if (gp<=1) (PIND & gpBit[gp]) ? buttons[gp] &= ~btnBits_NES[btn] : buttons[gp] |= btnBits_NES[btn];
              if (gp>1) (PINB & gpBit[gp]) ? buttons[gp] &= ~btnBits_NES[btn] : buttons[gp] |= btnBits_NES[btn];
            sendClock();
          }
